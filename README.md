...
- The Panel runtime session (above the unchanged shared VSCP client) stops online visualization polling after five consecutive failed UPDATE/CONFIG/CONTROL requests (timeout, invalid response, UID mismatch, or status=0). A successful runtime response resets the counter. A centered DISCONNECT panel appears regardless of debug settings; Reconnect performs INIT then CONNECT before resuming. History and the current recording are retained.
