...
VSCP (Virtual Sensors Communication Protocol) je jednoduchý textový protokol pro komunikaci mezi HMI/firmwarem EduBox HUB Panel a cílovou deskou, reálným zařízením nebo emulátorem. Aktuální implementace projektu používá VSCP API `1.6`.
...
| `api` | ano pro aktuální flow | VSCP API verze, aktuálně `1.6` |
...
```text
?status=0&error=API mismatch - got 1.2, expected 1.6
```
